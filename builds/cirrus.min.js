function renderProcess(c,e){importClass("VProcess");var d=new VProcess(theRoot),f=e.fields;d.setProcess(c);delete e.body;delete e.fields;for(var g=Object.keys(e),h=g.length;h--;)g[h].toUpperCase().match(/^[A-Z0-9_]*$/)&&(e[g[h]]instanceof Date?d.setVar(g[h],e[g[h]].toISOString().split("T")[0]):d.setVar(g[h].toUpperCase(),e[g[h]]));d.exec();h=d.varToString("RESULT");g="Content-Type: text/html; charset=utf-8";if(""===h){var l=d.result(),g="Content-Type: application/json";2===d.objectInfo().outputType()&&
0<l.size()?h=JSON.stringify(vRegisterListToJSON(l,f)):1===d.objectInfo().outputType()&&(d=new VRegisterList(theRoot),d.setTable(l.tableInfo().idRef()),d.append(l),h=JSON.stringify(vRegisterListToJSON(d,f)))}f=["Date: "+(new Date).toGMTString(),"Content-Length: "+h.length];f=f.concat(BasicHeaders).concat([g]);return"HTTP/1.0 200 OK\r\n"+f.join("\r\n")+"\r\n\r\n"+h}
...
(f=c.split("T"),e=f[0],d=f[1].split(".")[0],f=parseInt(e.split("-")[0],10),c=parseInt(e.split("-")[1],10)-1,e=parseInt(e.split("-")[2],10),hour=parseInt(d.split(":")[0],10),g=parseInt(d.split(":")[1],10),d=parseInt(d.split(":")[2],10),c=new Date(f,c,e,hour,g,d));return c}var BasicHeaders=["Server: Velneo v7","Access-Control-Allow-Origin: *","Access-Control-Allow-Headers: DNT,X-Mx-ReqToken,Keep-Alive,User-Agent,X-Requested-With,If-Modified-Since,Cache-Control,Content-Type","Access-Control-Allow-Methods: GET, POST, PUT, HEAD, OPTIONS, DELETE"];
