...
if(c=c.match(RegExp(wApp.session.cookie_name+"=(\\w+)\\;?")))d[e]=c[1],a=Base64.decode(d[e]),b=decodeURIComponent(a),d.session=JSON.parse(b.replace(/\0/g,"")),this.cookie=d,this.session=d.session,void 0!==this.session.flash&&(this.flashGet=this.session.flash,delete this.session.flash,this.changed=!0);return d[e]}},params:function(){return this.router.params},request:Request,response:Response,logError:logError,getHTML:getHTML,vRegisterListToJSON:vRegisterListToJSON,renderProcess:renderProcess,renderQuery:renderQuery,
...
f.encodeParams)for(;param=k.exec(g);)f.decodeParams[param[1]]=wApp.getType(decodeURIComponent(param[2]).replace(/\+/g," "));if(2==e.length)for(g=f.body=e[1].trim().replace(/\+/g," ");body=k.exec(g);)"_method"==body[1]&&decodeURIComponent(body[2]).match(/^(PUT|DELETE)$/i)?f.verb=decodeURIComponent(body[2]).toUpperCase():(e=f.bodyDecoded[body[1]],h=wApp.getType(decodeURIComponent(body[2])),void 0==e?f.bodyDecoded[body[1]]=h:("object"!=typeof e&&(f.bodyDecoded[body[1]]=[e]),f.bodyDecoded[body[1]].push(h)));
return f}
function Request(c){try{var d=http_parser(c);wApp.router.params=d.decodeParams;void 0!==d.bodyDecoded&&(wApp.router.params.body=d.bodyDecoded);void 0!==d.headers["Content-Type"]&&d.headers["Content-Type"].match(/application\/json/i)&&d.body?wApp.router.params.body=JSON.parse(d.body):void 0!==d.headers["Content-Type"]&&d.headers["Content-Type"].match(/text\/xml/i)&&d.body?wApp.router.params.body=d.body:wApp.router.params.body=d.bodyDecoded;void 0!==d.headers.Cookie&&wApp.session.getFromHeader(d.headers.Cookie);return d}catch(e){return e}}
function getType(c){if(c.match(/^[1-9]\d*$/i)&&14>c.length)return parseInt(c,10);if(c.match(/^\d{1,3}(\.\d{3})*(\,\d*)?$/g))return parseFloat(c.replace(/\./g,"").replace(",","."));if(c.match(/^\d{1,3}(\,\d{3})*(\.\d*)?$/g))return parseFloat(c.replace(/\,/g,""));if(c.match(/^\d*,\d*$/i))return parseFloat(c.replace(",","."));if(c.match(/^\d*\.\d*$/i))return parseFloat(c);if(c.match(/(true|false)/))return"true"==c?!0:!1;if(c.match(/^\d{1,2}[\/-]\d{1,2}[\/-]\d{2,4}$/)){var d=c.split("/"),e=c.split("-"),
f=1<d.length?d:e;return c=new Date(parseInt(f[2],10),parseInt(f[1],10)-1,parseInt(f[0],10))}if(c.match(/^\d{4}[\/-]\d{1,2}[\/-]\d{1,2}$/))return d=c.split("/"),e=c.split("-"),f=1<d.length?d:e,c=new Date(parseInt(f[0],10),parseInt(f[1],10)-1,parseInt(f[2],10));if(c.match(/^\d{1,2}[\/-]\d{1,2}[\/-]\d{2,4} \d{1,2}:\d{1,2}:\d{1,2} \w\w$/))return f=c.split(" "),c=f[0].split("/"),d=f[1].split(":"),f=f[2].match(/am/i)?parseInt(d[0]):parseInt(d[0])+12,c=new Date(parseInt(c[2],10),parseInt(c[0],10)-1,parseInt(c[1],
10),f,parseInt(d[1],10),parseInt(d[2],10));if(c.match(/^\w*, \d* \w* \d{2,4} \d{1,2}:\d{1,2}:\d{1,2} \w\w\w$/)){f=c.split(",")[1].split(" ");d=parseInt(f[0],10);c={Jan:1,Feb:2,Mar:3,Apr:4,May:5,Jun:6,Jul:7,Aug:8,Sep:9,Oct:10,Nov:11,Dec:12}[f[1]];var f=parseInt(f[2],10),g=parseInt(parmas[3].split(":")[0],10),h=parseInt(parmas[3].split(":")[1],10),e=parseInt(parmas[3].split(":")[2],10);return c=new Date(f,c,d,g,h,e)}c.match(/^\d{2,4}-\d{1,2}-\d{1,2}T\d{1,2}:\d{1,2}:\d{1,2}\.\d*Z$/)&&(f=c.split("T"),
d=f[0],e=f[1].split(".")[0],f=parseInt(d.split("-")[0],10),c=parseInt(d.split("-")[1],10)-1,d=parseInt(d.split("-")[2],10),g=parseInt(e.split(":")[0],10),h=parseInt(e.split(":")[1],10),e=parseInt(e.split(":")[2],10),c=new Date(f,c,d,g,h,e));return c}var BasicHeaders=["Server: Velneo v7","transfer-coding: chunked","Keep-Alive: timeout=5, max=94","Connection: Keep-Alive"];function isAsset(c){return"js"===c.extension||"css"===c.extension}
function Response(c){try{if("SyntaxError"==c.name)throw c;if("js"===c.extension||"css"===c.extension){var d=getHTML(c.url);if(""!==d.html){var e="css"===c.url.substr(c.url.length-3)?"text/css":"application/javascript";return renderResponseAssets(d,e)}return"HTTP/1.0 404 NOT FOUND"}if("pro"===c.extension){var f=wApp.router.pointRequest(c.verb+" "+c.url);return"NOT FOUND"!=f?renderProcess(f,wApp.router.params):"HTTP/1.0 404 NOT FOUND"}if("bus"===c.extension){var g=wApp.router.pointRequest(c.verb+" "+
...
"application";d="/views/"+e.replace("Controller","")+"/"+f;c.controller=e;c.action=f;c.session=wApp.session.session;c.flash=wApp.session.flashGet;!1!==c.layout?(e=getHTML("/layouts/"+g),"template"===e.type&&eval("layout_temp = "+e.html),e="template"===e.type?Handlebars.VM.template(layout_temp)(c):e.html):e="#yield";d=getHTML(d);"template"===d.type&&eval("template = "+d.html);""==d.html&&(d.html="<div><h1>There is not view for this action</h1></div>");c="template"===d.type?Handlebars.VM.template(template)(c):
d.html;c=e.replace("#yield",c)}return{verb:"HTTP/1.0 200 OK",body:unescape(encodeURIComponent(c)),headers:["Content-Type: text/html; charset=utf-8"]}},xml:function(c){return{verb:"HTTP/1.0 200 OK",body:c.xml?unescape(encodeURIComponent(c.xml)):"<?xml version='1.0' encoding='UTF-8'?><error version='1.0'>The JSON object should have an xml key</error>",headers:["Content-Type: text/xml; charset=UTF-8"]}},redirect:function(c){return{verb:"HTTP/1.0 302 Found",body:"",headers:["location: "+c.redirect_to]}}};
function getHTML(c){importClass("VTextFile");importClass("VFile");var d=null==c.split(".")[c.split(".").length-1].match(/(css|js|html)/i)?".html":"";c+=d;d=new VTextFile(wApp.config.root_path+c);c=new VTextFile(wApp.config.root_path+c+".hbs");var e=c.exists()?"template":"html";html=d.exists()?d.open(VFile.OpenModeReadOnly)?d.readAll():"":c.exists()?c.open(VFile.OpenModeReadOnly)?c.readAll():"":"";return{html:html,type:e}}
