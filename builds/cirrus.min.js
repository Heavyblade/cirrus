...
function vRegisterListToJSON(c,d){var e=c.tableInfo(),f=e.fieldCount(),g=c.size(),h=[],l=[],k=0;if(void 0===d)for(;f--;)k=parseInt(e.fieldType(f),10),k=11===k?parseInt("11"+e.fieldObjectType(f)):k,l.push({fieldName:e.fieldId(f),fieldType:k});else for(d="string"==typeof d?d.toUpperCase().split(","):d;f--;)-1<d.indexOf(e.fieldId(f))&&l.push({fieldName:e.fieldId(f),fieldType:e.fieldType(f)});for(f=l.length;g--;){for(var e=c.readAt(g),k=f,m={};k--;)m[l[k].fieldName]=mapField(l[k].fieldType,l[k].fieldName,
e);h.push(m)}return h}
