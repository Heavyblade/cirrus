...
f.encodeParams)for(;param=l.exec(g);)k=decodeURIComponent(param[1]).replace("[]",""),h=wApp.getType(decodeURIComponent(param[2]).replace(/\+/g," ")),m=f.decodeParams[k],void 0===m?f.decodeParams[k]=h:("object"!==typeof m&&(f.decodeParams[k]=[m]),f.decodeParams[k].push(h));if(2==e.length)for(g=f.body=e[1].trim().replace(/\+/g," ");body=l.exec(g);)"_method"==body[1]&&decodeURIComponent(body[2]).match(/^(PUT|DELETE)$/i)?f.verb=decodeURIComponent(body[2]).toUpperCase():(e=f.bodyDecoded[body[1]],h=wApp.getType(decodeURIComponent(body[2])),
void 0===e?f.bodyDecoded[body[1]]=h:("object"!=typeof e&&(f.bodyDecoded[body[1]]=[e]),f.bodyDecoded[body[1]].push(h)));return f}
