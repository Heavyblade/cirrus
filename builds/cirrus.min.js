...
(f=c.split("T"),d=f[0],e=f[1].split(".")[0],f=parseInt(d.split("-")[0],10),c=parseInt(d.split("-")[1],10)-1,d=parseInt(d.split("-")[2],10),hour=parseInt(e.split(":")[0],10),g=parseInt(e.split(":")[1],10),e=parseInt(e.split(":")[2],10),c=new Date(f,c,d,hour,g,e));return c}var BasicHeaders=["Server: Velneo v7"];function isAsset(c){return"js"===c.extension||"css"===c.extension}
...
h){var k=h.split("#");return renderResponse(k[0],k[1],wApp,c.extension||c.headers.Accept)}return"HTTP/1.0 404 NOT FOUND"}catch(l){switch(l.name){case "SyntaxError":return renderErrorResponse(l,"Unable to parse JSON string");default:return c=logError(l),renderErrorResponse(l,c)}}}
