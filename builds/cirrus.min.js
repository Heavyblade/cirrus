...
function getHTML(c){importClass("VTextFile");importClass("VFile");var d=null==c.split(".")[c.split(".").length-1].match(/(css|js|html)/i)?".html":"";c+=d;d=new VTextFile(wApp.config.root_path+c);c=new VTextFile(wApp.config.root_path+c+".hbs");d=d.exists()?d.readAll():c.exists()?c.readAll():"<div><h1>There is not view for this action</h1></div>";c=c.exists()?"template":"html";return{html:d,type:c}}
