...
wApp.router.params):"HTTP/1.0 404 NOT FOUND"}if("OPTIONS"===a.verb)return"HTTP/1.0 200 OK\r\nAllow: HEAD,GET,PUT,DELETE,OPTIONS\r\n\r\n";var f=wApp.router.pointRequest(a.verb+" "+a.url.split(".")[0]);if("NOT FOUND"!=f){var g=f.split("#");return renderResponse(g[0],g[1],wApp,a.extension||a.headers.Accept)}return"HTTP/1.0 404 NOT FOUND"}catch(h){return a=logError(h),renderErrorResponse(h,a)}}
function renderResponseAssets(a,c){var b=["Date: "+(new Date).toGMTString(),"Content-Length: "+a.length],b=b.concat(BasicHeaders).concat(["Content-Type: "+c]);return"HTTP/1.0 200 OK\r\n"+b.join("\r\n")+"\r\n\r\n"+a}
...
var Engine={json:function(a,c){var b=void 0!==a.responseCode&&void 0!==a.responseCode.code&&void 0!==a.responseCode.message?"HTTP/1.0 "+a.responseCode.code+" "+a.responseCode.message:"HTTP/1.0 200 OK",d=c.router.params.callback;a=d?d+"("+JSON.stringify(a)+")":JSON.stringify(a);a=unescape(encodeURIComponent(a));return{verb:b,body:a,headers:["Content-Type: application/"+(d?"javascript":"json")+"; charset=utf-8"]}},html:function(a,c,b,d){c=a.layout||"application";b="/views/"+b.replace("Controller","")+
"/"+d;!1!==a.layout?(d=getHTML("/layouts/"+c),"template"==d.type&&eval("layout_temp = "+d.template),d="template"==d.type?Handlebars.VM.template(layout_temp)(a):d.html):d="#yield";b=getHTML(b);"template"==b.type&&eval("template = "+b.template);a="template"==b.type?Handlebars.VM.template(template)(a):b.html;a=d.replace("#yield",a);return{verb:"HTTP/1.0 200 OK",body:unescape(encodeURIComponent(a)),headers:["Content-Type: text/html; charset=utf-8"]}},redirect:function(a){return{verb:"HTTP/1.0 302 Found",
body:"",headers:["location: "+a.redirect_to]}}};function getHTML(a){var c=new VRegisterList(theRoot);c.setTable("cirrusdat/FILES_MEM");c.load("PATH",[a]);if(0<c.listSize()){var b=c.readAt(0);a=b.fieldToString("BODY");c="1"==b.fieldToString("TIPO")?"html":"template";b=b.fieldToString("COMPILED")}else c.load("NAME",["NOT_VIEW"]),a=c.readAt(0).fieldToString("BODY"),b="",c="html";return{html:a,type:c,template:b}}
function logError(a){return void 0===a.lineNumber?a.message:a.message+". In Line Number: "+a.lineNumber}function renderErrorResponse(a,c){var b=wApp.router.params.callback,d={message:c},d=b?b+"("+JSON.stringify(d)+")":JSON.stringify(d),d=unescape(encodeURIComponent(d));return"HTTP/1.0 500 INTERNAL SERVER ERROR\r\n"+BasicHeaders.join("\r\n")+"\r\n\r\n"+d}
