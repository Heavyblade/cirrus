...
h){var k=h.split("#");return renderResponse(k[0],k[1],wApp,c.extension||c.headers.Accept)}var l=getHTML("/views"+c.url);if(""!==l.html){var m=getHTML("/layouts/application"),p=(""===m.html?"#yield":m.html).replace("#yield",l.html);return renderResponseAssets({html:p},"text/html; charset=utf-8")}return"HTTP/1.0 404 NOT FOUND"}catch(n){switch(n.name){case "SyntaxError":return renderErrorResponse(n,"Unable to parse JSON string");default:return c=logError(n),renderErrorResponse(n,c)}}}
