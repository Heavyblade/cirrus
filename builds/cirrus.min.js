...
function vRegisterListToJSON(c,e){var d=c.tableInfo(),f=d.fieldCount(),g=c.size(),h=[],l=[],k=0;if(void 0===e)for(;f--;)k=parseInt(d.fieldType(f),10),k=11===k?parseInt("11"+d.fieldObjectType(f)):k,l.push({fieldName:d.fieldId(f),fieldType:k});else for(e=e.toUpperCase().split(",");f--;)-1<e.indexOf(d.fieldId(f))&&l.push({fieldName:d.fieldId(f),fieldType:d.fieldType(f)});for(f=l.length;g--;){for(var d=c.readAt(g),k=f,m={};k--;)m[l[k].fieldName]=mapField(l[k].fieldType,l[k].fieldName,d);h.push(m)}return h}
...
function Response(c){try{if("SyntaxError"==c.name)throw c;var e={js:"application/javascript",css:"text/css",woff:"application/font-woff",ttf:"application/font-sfnt",svg:"image/svg+xml",eot:"application/vnd.ms-fontobject"};if(void 0!==e[c.extension]){var d=getHTML(c.url);return""!==d.html?renderResponseAssets(d,e[c.extension]):"HTTP/1.0 404 NOT FOUND"}if("pro"===c.extension){var f=wApp.router.pointRequest(c.verb+" "+c.url);return"NOT FOUND"!=f?renderProcess(f,wApp.router.params):"HTTP/1.0 404 NOT FOUND"}if("bus"===
...
function renderUnauthorized(){var c=unescape(encodeURIComponent(JSON.stringify({message:"Unauthorized"})));return"HTTP/1.0 401 Unauthorized\r\n"+BasicHeaders.concat(['WWW-Authenticate: Basic realm="cirrus"']).join("\r\n")+"\r\n\r\n"+c}function isAuthorized(c,e){var d=Base64.decode(e.headers.Authorization.split(" ")[1]).split(":");return c.authentication.username==d[0]&&c.authentication.password==d[1]}
function renderResponse(c,e,d,f){var g=f.extension||f.headers.Accept,h=void 0!==d[c].authentication&&"object"==typeof d[c].authentication,l=-1<((d[c].authentication||{}).actions||[]).indexOf(e)||(d[c].authentication||{}).all;if(h&&l&&(void 0===f.headers.Authorization||!1===isAuthorized(d[c],f)))return renderUnauthorized();void 0!==d[c].before&&"function"==typeof d[c].before&&(d.router.params=d[c].before(d.router.params));f=void 0!==d.router.params.redirect_to?d.router.params:d[c][e](d.router.params);
g=g||"json";f.redirect_to&&(g="redirect");g=g.match(/(html|xml|json|redirect)/i)||["json"];c=Engine[g[0]](f,d,c,e);e=["Date: "+(new Date).toGMTString(),"Content-Length: "+(c.body?c.body.length:"0")];e=e.concat(BasicHeaders).concat(c.headers).concat(d.responseHeaders.buildForHTTP());wApp.session.changed&&e.push(wApp.session.setInHeader());return c.verb+"\r\n"+e.join("\r\n")+"\r\n\r\n"+c.body}
...
"application";e="/views/"+d.replace("Controller","")+"/"+f;var h;c.controller=d;c.action=f;c.session=wApp.session.session;c.flash=wApp.session.flashGet;!1!==c.layout?(d=getHTML("/layouts/"+g),"template"===d.type&&(layout_temp=Handlebars.compile(d.html)),d="template"===d.type?layout_temp(c):d.html):d="#yield";e=getHTML(e);"template"===e.type&&(h=Handlebars.compile(e.html));""===e.html&&(e.html="<div><h1>There is not view for this action</h1></div>");c="template"===e.type?h(c):e.html;c=d.replace("#yield",
c)}return{verb:"HTTP/1.0 200 OK",body:c,headers:["Content-Type: text/html; charset=utf-8"]}},xml:function(c){return{verb:"HTTP/1.0 200 OK",body:c.xml?c.xml:"<?xml version='1.0' encoding='UTF-8'?><error version='1.0'>The JSON object should have an xml key</error>",headers:["Content-Type: text/xml; charset=UTF-8"]}},redirect:function(c){return{verb:"HTTP/1.0 302 Found",body:"",headers:["location: "+c.redirect_to]}}};
