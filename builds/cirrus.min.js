...
function vRegisterListToJSON(c,d){var e=c.tableInfo(),f=e.fieldCount(),g=c.size(),h=[],l=[],k=0;if(void 0===d)for(;f--;)k=parseInt(e.fieldType(f),10),k=11===k?parseInt("11"+e.fieldObjectType(f)):k,l.push({fieldName:e.fieldId(f),fieldType:k});else for(d=d.toUpperCase().split(",");f--;)-1<d.indexOf(e.fieldId(f))&&l.push({fieldName:e.fieldId(f),fieldType:e.fieldType(f)});for(f=l.length;g--;){for(var e=c.readAt(g),k=f,m={};k--;)m[l[k].fieldName]=mapField(l[k].fieldType,l[k].fieldName,e);h.push(m)}return h}
...
f.encodeParams)for(;param=l.exec(g);)f.decodeParams[param[1]]=wApp.getType(decodeURIComponent(param[2]).replace(/\+/g," "));if(2==e.length)for(g=f.body=e[1].trim().replace(/\+/g," ");body=l.exec(g);)"_method"==body[1]&&decodeURIComponent(body[2]).match(/^(PUT|DELETE)$/i)?f.verb=decodeURIComponent(body[2]).toUpperCase():(e=f.bodyDecoded[body[1]],h=wApp.getType(decodeURIComponent(body[2])),void 0===e?f.bodyDecoded[body[1]]=h:("object"!=typeof e&&(f.bodyDecoded[body[1]]=[e]),f.bodyDecoded[body[1]].push(h)));
return f}
...
h){var l=h.split("#");return renderResponse(l[0],l[1],wApp,c.extension||c.headers.Accept)}var k=getHTML("/views"+c.url);if(""!==k.html){var m=getHTML("/layouts/application"),p=(""===m.html?"#yield":m.html).replace("#yield",k.html);return renderResponseAssets({html:p},"text/html; charset=utf-8")}return"HTTP/1.0 404 NOT FOUND"}catch(n){switch(n.name){case "SyntaxError":return renderErrorResponse(n,"Unable to parse JSON string");default:return c=logError(n),renderErrorResponse(n,c)}}}
