...
"")]=a[f.indexOf(c)]}return d[e]}}return"NOT FOUND"}},cookie:{session:{}},session:{},oldcookie:{session:{}},sessionChanged:!1,setInSession:function(a,b){this.sessionChanged=!0;wApp.session[a]=b},setSession:function(){var a=0!=Object.keys(this.session).length?Base64.encode(encodeURIComponent(JSON.stringify(this.session))):"";if(0!=Object.keys(this.session).length){var b=this.session.expires,c=this.session.path;delete this.session.expires;delete this.session.path;a=Base64.encode(encodeURIComponent(JSON.stringify(this.session)))}else c=
b=void 0;a="value="+a;void 0!=b&&(a+="; expires="+b.toGMTString());void 0!=c&&(a+="; path="+c);return"set-Cookie: "+a},getSession:function(a){a=a.split("; ");for(var b=a.length,c={},d={};b--;){var e=a[b].split("=");c[e[0]]=e[1];d[e[0]]=e[1]}d.session=c.session=JSON.parse(decodeURIComponent(Base64.decode(c.value)));this.cookie=c;this.oldcookie=d;this.session=c.session;return c.value},params:function(){return this.router.params}};
...
param[2];if(2==a.length)for(b.body=decodeURIComponent(a[1].trim().replace(/\+/g," "));body=e.exec(a[1]);)b.bodyDecoded[body[1]]=body[2];return b}function Request(a){a=http_parser(a);wApp.router.params=a.decodeParams;wApp.router.params.body=a.bodyDecoded;void 0!=a.headers.Cookie&&wApp.getSession(a.headers.Cookie);return a}function Response(a){a=wApp.router.pointRequest(a.verb+" "+a.url);"NOT FOUND"!=a?(a=a.split("#"),a=renderResponse(a[0],a[1],wApp)):a="HTTP/1.1 404 NOT FOUND";return a}
